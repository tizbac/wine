...
static void surface_realize_palette(struct wined3d_surface *surface)
{
            ...
            || surface->resource.format->id == WINED3DFMT_P8_UINT_A8_UNORM)
    {
        ...
        {
            if (!(surface->resource.locations & surface->map_binding))
            {
                TRACE("Palette changed with surface that does not have an up to date system memory copy.\n");
                surface_prepare_map_memory(surface);
                surface_load_location(surface, context, surface->map_binding);
            }
    ...
    if (surface->resource.usage & WINED3DUSAGE_RENDERTARGET)
        surface_load_location(surface, context, surface->draw_binding);
...
HRESULT CDECL wined3d_surface_releasedc(struct wined3d_surface *surface, HDC dc)
{
    ...
    if (surface->map_binding == WINED3D_LOCATION_USER_MEMORY)
    {
        ...
        if (device->d3d_initialized)
            context = context_acquire(device, NULL);
        surface_load_location(surface, context, WINED3D_LOCATION_USER_MEMORY);
        if (context)
            context_release(context);
    }
                ...
                else
                {
                    ...
                    if (SUCCEEDED(surface_upload_from_surface(dst_surface, &dst_point, src_surface, &src_rect)))
                    {
                        if (!surface_is_offscreen(dst_surface))
                        {
                            ...
                            surface_load_location(dst_surface, context, dst_surface->draw_binding);
                            context_release(context);
                        }
                        return WINED3D_OK;
                    }
